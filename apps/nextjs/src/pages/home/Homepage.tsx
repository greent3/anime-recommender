...
function Homepage() {
    ...
    return (
        <div className=' themed-centered-stack-fullwidth-fullheight pt-12'>
            ...
            {!exampleSeries.isFetching && exampleSeries.data && <SeriesDetails
                title={exampleSeries?.data?.title || "Unknown Title"}
                type={exampleSeries?.data?.type} bio={exampleSeries?.data?.bio}
            ...
            />}
            {pageToShow == 'inquiry' && !exampleSeries.isFetching && exampleSeries.data && <InquiryButtonRow goToRating={goToRating} goToWatchlist={goToWatchlist} />}
            {pageToShow == 'addAction' && !exampleSeries.isFetching && exampleSeries.data && <AddToWatchlistButtonRow goToInquiry={goToInquiry} seriesId={exampleSeries.data?.id} />}
            {pageToShow == 'rating' && !exampleSeries.isFetching && exampleSeries.data && <RatingButtonRow goToInquiry={goToInquiry} seriesId={exampleSeries.data?.id} />}
        </div>
    )
}
